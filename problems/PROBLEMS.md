...
8. Login dropped to the passcode screen after closing the browser (mobile Safari especially), even inside the configured 30-day window — `AccountController.Login` called `SignInAsync` with no `AuthenticationProperties`, so `IsPersistent` defaulted to false and the cookie was session-only → `AuthCookieRules.SignInProperties()` sets `IsPersistent = true` (med-history-4ei.29)
9. Thai readers still met English in three places after the whole app was translated — the confirm dialog's Cancel/Confirm, the lightbox's `aria-label="Close"`, the checklist's "0 of 3 meds done" and every allocation description — shared partials belong to no slice, and an interpolated sentence has no key left to look up → own `.resx` per partial, `ChecklistRules.Progress`/`MedStockRules.RemainingLabel` return key+args, descriptions split back on `MedPlanRules.PartSeparator` (med-history-4ei.31.11)
