...
- Med checklist: `MedAllocations` per-day rows — Name + `Slots` ([Flags] Morning/Noon/Evening/Bedtime, stored canonical CSV via explicit converter) + MealRelation + Method (enum-as-string). Ticks are PER SLOT: tick creates a Pill entry linked via `Entry.ChecklistAllocationId`+`ChecklistSlot` (nullable, NO FK — a logged dose outlives its plan), note composed by `MedPlanRules.ComposeNote`; untick deletes exactly the linked entry (day-scoped). Slot state = linked entry exists; manual Pill entries do NOT count. Dangling allocation ids are tolerated everywhere. Maintenance at `/day/{date}/meds` (`MedsController`); tick/untick on `DayController`. Plan vocabulary pure fns in `MedPlanRules`, behavior in `ChecklistRules`
